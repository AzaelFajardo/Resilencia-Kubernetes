...
def _build_sampler():
    ...
    # env vars. Only the SDK's own auto-instrumentation CLI reads these
    # automatically; this project builds its TracerProvider by hand, so it
    # never did until now (deferred since Phase 5 specifically to measure
    # sampling's impact - see docs/tests/sampling-results.md).
