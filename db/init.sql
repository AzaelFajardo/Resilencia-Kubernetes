...
-- -----------------------------------------------------------------------
-- 6. SERVICE LEADER  (distributed lock for the order orchestrator)
-- -----------------------------------------------------------------------
-- One row per candidate service (order/payment/inventory/user/notification).
-- Alive candidates heartbeat their row; the leader = highest priority among
-- alive, tie-broken by node id. Leader election runs only inside Kubernetes
-- (auto-detected via KUBERNETES_SERVICE_HOST).
-- -----------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS service_leader (
